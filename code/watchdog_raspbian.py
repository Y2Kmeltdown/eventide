# Raspberry Pi hardware watchdog feeder — uses gpiozero, whose default pin
# factory (RPi.GPIO/lgpio autodetect) only works on Raspberry Pi boards. For
# other SBCs (e.g. Orange Pi) see watchdog_orangepi.py, which talks to the
# same I2C watchdog IC via the generic libgpiod interface instead. install.sh
# copies whichever of these matches the detected board to watchdog.py at
# install time (see os_services_raspbian / os_services_orangepi).
import gpiozero
import smbus2
import time
