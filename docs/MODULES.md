...
| `form`     | sidebar | `get`, `put`, `method?` (`"PUT"` default, or `"POST"`), `submit_label?`, `fields[]`. GET populates, PUT/POST applies. Field: `{key, label?, kind: number\|slider\|toggle\|text\|select, min?, max?, step?, get?, put?, options?}` — per-field `get`/`put` overrides let one form span several endpoints. |
